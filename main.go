...
//テンプレートファイルの配列を作成、{"string1": pointer1, "string2": pointer2 }
...
func topHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, file := range files {
		if strings.HasSuffix(file.Name(), expendString) {
			...
			fmt.Println(strings.Split(string(file.Name()), expendString))
...
func (p *Page) save() error {
	//タイトルの名前でテキストファイルを作成して保存
	filename := p.Title + ".txt"
	//0600は、テキストデータを書き込んだり読み込んだりする権限を設定している
